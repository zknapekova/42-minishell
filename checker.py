...
for i in range(len(test_cases)):
    minishell_run = subprocess.run(f"./minishell {test_cases[0]}", shell=True, text=True, capture_output=True)
    minishell_output = minishell_run.stdout.strip()

    cmd = f"export {test_cases[0]} && env"
    all_env_vars = subprocess.run(cmd, shell=True, text=True, capture_output=True)

    if (minishell_output in all_env_vars.stdout.strip()):
        print(f"✅ Test {test_cases[0]} passed! ✅")
    else:
        print(f"❌ Test {test_cases[0]} failed! ❌")

    valgrind_process = subprocess.run(f"valgrind ./minishell {test_cases[0]}", shell=True, text=True,
                                      capture_output=True)
