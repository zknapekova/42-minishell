...
/*   Updated: 2025/04/17 16:27:52 by jgrigorj         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	align_dec_left(int dec, char *flags, int *wp, int *ch_printed)
{
	int		p;
	char	*dec_str;

	p = 0;
	eprint_sign(&dec, flags, ch_printed);
	dec_str = ft_itoa(dec);
	if (dec == -2147483648)
		ft_strlcpy(dec_str, "2147483648", 11);
	while (p < wp[1] - (int)ft_strlen(dec_str))
		eprint_padding('0', ch_printed, &p);
	ewrite_number(dec_str, dec, ch_printed, wp[1]);
	while (*ch_printed < wp[0])
		eprint_padding(' ', ch_printed, &p);
	free (dec_str);
}

static void	align_dec_right(int dec, char *flags, int *wp, int *ch_printed)
{
	int		p;
	int		dummy;
	char	*dec_str;
	...
	if (wp[1] >= 0 || flags[2] != 'z')
		eprint_space_on_left(dec, ch_printed, wp);
	eprint_sign(&dec, flags, ch_printed);
	free (dec_str);
	dec_str = ft_itoa(dec);
	if (dec == -2147483648)
