...
/*   Updated: 2025/04/17 16:21:05 by jgrigorj         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_eprintf.h"

static int	get_ptr_len(uintptr_t num)
{
	int	len;
...
static void	align_ptr_left(unsigned long long ptr, int width, int *ch_printed)
{
	int	dummy;

	dummy = 0;
	if (!ptr)
		*ch_printed += write(2, "(nil)", 5);
	else
	{
		*ch_printed += write(2, "0x", 2);
		put_ptr(ptr);
...
static void	align_ptr_right(unsigned long long ptr, int width, int *ch_printed)
{
	int	len;
	int	dummy;
