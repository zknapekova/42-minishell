...
/*   Updated: 2025/04/17 16:21:42 by jgrigorj         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	align_undec_left(unsigned int undec, int *wp, int *ch_printed)
{
	int		p;
	char	*undec_str;

	p = 0;
	undec_str = euitoa(undec);
	while (p < wp[1] - (int)ft_strlen(undec_str))
		eprint_padding('0', ch_printed, &p);
	ewrite_number(undec_str, undec, ch_printed, wp[1]);
	while (*ch_printed < wp[0])
		eprint_padding(' ', ch_printed, &p);
	free (undec_str);
}

static void	align_undec_right(int undec, char *flags, int *wp, int *ch_printed)
{
	int		p;
	char	*undec_str;
	...
	if (wp[1] < 0 && flags[2] == 'z')
		while (((*ch_printed < wp[0] - (int)ft_strlen(undec_str))))
			eprint_padding('0', ch_printed, &p);
	euprint_space_on_left(undec, ch_printed, wp);
	while (((p < wp[1] - (int)ft_strlen(undec_str))))
		eprint_padding('0', ch_printed, &p);
	ewrite_number(undec_str, undec, ch_printed, wp[1]);
	free (undec_str);
